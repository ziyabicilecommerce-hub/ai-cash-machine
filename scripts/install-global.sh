...
# 2) ALL skills, from both trees this repo keeps them in (-L dereferences
#    any symlinks so only real files land in the global folder). Remove
#    stale broken symlinks in the target first.
find "$CLAUDE_HOME/skills" -maxdepth 1 -type l -delete 2>/dev/null
cp -rL "$REPO_ROOT/.claude/skills/." "$CLAUDE_HOME/skills/" 2>/dev/null
cp -rL "$REPO_ROOT/.agents/skills/." "$CLAUDE_HOME/skills/" 2>/dev/null
SKILL_N=$(ls -d "$CLAUDE_HOME"/skills/*/ 2>/dev/null | wc -l | tr -d ' ')
echo "  ✓ $SKILL_N skills (incl. ruflo, ui-ux-pro-max) -> $CLAUDE_HOME/skills"
