...
async function main() {
  ...
  if (!filings) {
    console.log('[81-insider-buy-radar] Kein SEC-Tagesindex für heute/gestern verfügbar (Feiertag/Wochenende?).');
    return;
  }
  ...
  if (!neueFilings.length) {
    console.log('[81-insider-buy-radar] Keine neuen Form-4-Meldungen seit dem letzten Lauf.');
    return;
  }
  ...
  for (const filing of zuVerarbeiten) {
    ...
    try {
      ...
      console.error(`[81-insider-buy-radar] Filing ${filing.accessionNoDash} übersprungen:`, err.message || err);
    }
  ...
  if (!kandidaten.length) {
    console.log(`[81-insider-buy-radar] ${zuVerarbeiten.length} Form-4-Meldungen geprüft, keine über der Schwelle (min. ${minKaufwertUsd} USD oder ${minInsiderAnzahl}+ Insider).`);
    return;
  }
  ...
  console.log(`[81-insider-buy-radar] ${kandidaten.length} Insider-Kauf-Signal(e) per WhatsApp verschickt (${zuVerarbeiten.length} von ${neueFilings.length} neuen Meldungen geprüft).`);
}

main().catch((err) => {
  if (err?.uebersprungen) {
    console.log('[81-insider-buy-radar] Übersprungen:', err.message);
    process.exit(0);
  }
  console.error('[81-insider-buy-radar] Fehler:', err);
  process.exit(1);
});
