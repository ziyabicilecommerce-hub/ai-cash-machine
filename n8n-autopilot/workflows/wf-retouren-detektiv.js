...
const scheduleTrigger = trigger({
  ...
  version: 1.3,
  config: { name: 'Wöchentlich Dienstag 09:00', parameters: { rule: { interval: [ { field: 'weeks', weeksInterval: 1, triggerAtDay: [2], triggerAtHour: 9, triggerAtMinute: 0 } ] } } },
  output: [{}]
});
...
const getOrders = node({
  ...
  version: 4.4,
  config: { name: 'Orders mit Rückerstattung', parameters: {
    method: 'GET',
    url: expr("https://{{ $('Setup').first().json.SHOP }}.myshopify.com/admin/api/2024-10/orders.json?status=any&limit=250&created_at_min={{ encodeURIComponent($now.minus({ days: 30 }).toISO()) }}&fields=id,line_items,refunds,total_price"),
    sendHeaders: true,
...
const analyze = node({
  ...
  version: 2,
  config: { name: 'Retouren-Muster finden', parameters: { mode: 'runOnceForAllItems', jsCode: `const NL = String.fromCharCode(10);
const setup = $('Setup').first().json;
const orders = $input.first().json.orders || [];
const produktRetouren = {};
let gesamtRetouren = 0;
let gesamtWert = 0;
for (const o of orders) {
  ...
  for (const r of refunds) {
    for (const li of (r.refund_line_items || [])) {
      const titel = (li.line_item || {}).title || 'unbekannt';
      produktRetouren[titel] = (produktRetouren[titel] || 0) + (li.quantity || 1);
    }
  }
  gesamtWert += parseFloat(o.total_price || 0);
}
const top = Object.entries(produktRetouren).sort((a,b)=>b[1]-a[1]).slice(0, 8).map(e => e[0] + ': ' + e[1] + 'x zurückgegeben').join(NL);
const prompt = 'Du bist Qualitäts-/Retouren-Analyst für den Onlineshop "' + setup.SHOP_NAME + '".' + NL + NL +
  'Letzte 30 Tage: ' + gesamtRetouren + ' Bestellungen mit Retoure.' + NL +
  'Top-Retouren-Produkte:' + NL + (top || '- keine Daten') + NL + NL +
  'Analysiere auf Deutsch:' + NL +
  '1. Welche Produkte fallen auf und WARUM könnten sie zurückgegeben werden? (Größe, Qualität, Erwartung vs. Realität)' + NL +
  '2. Konkrete Maßnahmen: Produktbeschreibung anpassen, Größentabelle, Fotos verbessern?' + NL +
  '3. Ist eine Qualitätsprüfung beim Lieferanten nötig?' + NL + NL +
...
const claude = node({
  type: 'n8n-nodes-base.httpRequest',
  version: 4.4,
  config: { name: 'Claude Retouren-Analyse', parameters: {
    method: 'POST',
...
const setup = $('Setup').first().json;
return { json: {
  betreff: 'Retouren-Detektiv-Report - ' + setup.SHOP_NAME,
...
export default workflow('retouren-detektiv', '21 · Retouren-Detektiv 🔍📦')
  .add(scheduleTrigger)
  .to(setup)
  .to(getOrders)
  .to(analyze)
