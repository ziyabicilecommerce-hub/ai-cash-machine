// Monte-Carlo-Simulation der EIGENEN Trade-Historie - Bootstrap-Resampling
// (zieht zufällig, mit Zurücklegen, aus den echten abgeschlossenen Trades
// dieses Symbols) simuliert tausende plausible Zukunftspfade, statt eine
// einzelne Prognose zu behaupten. Zeigt eine Bandbreite möglicher
// Ergebnisse (Perzentile) UND die Wahrscheinlichkeit, den eigenen
// Kill-Switch in den nächsten Trades zu erreichen.
//
...
// bisherigen - eine Annahme, die bei einer Strategie-/Marktänderung nicht
// mehr stimmen muss. Bei wenigen Trades entsprechend unzuverlässig (siehe
// MIN_TRADES). Rein informativ: verändert NIE Kapital oder Position, kein
// einziger Aufruf von entscheideKauf/entscheideVerkauf hier.
//
...
function simuliereEinenPfad(renditenProzent, startKapital, gesamtverlustSchwelleProzent) {
  ...
  for (let i = 0; i < ZUKUENFTIGE_TRADES; i++) {
    if (killSwitchGetroffen) break; // Bot würde ab hier ohnehin nicht mehr für dieses Symbol handeln
...
export async function pruefeUndAktualisiereMonteCarlo(env, cfg) {
  ...
  const letzte = await env.TRADING_STATE.get('montecarlo:letzteWoche');
  ...
  for (const symbol of cfg.symbols) {
    try {
      ...
      await env.TRADING_STATE.put(`montecarlo:${symbol}`, JSON.stringify({ ...ergebnis, berechnetAm: jetzt.toISOString() }));
    } catch (err) {
      console.error(`[trading-bot] Monte-Carlo ${symbol} fehlgeschlagen:`, err);
    }
  }
  await env.TRADING_STATE.put('montecarlo:letzteWoche', aktuelleWoche);
}
