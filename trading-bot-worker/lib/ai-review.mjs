...
// abgeschlossenen Trades durch Claude (Anthropic API), Pendant zum Muster
// in automations/lib/claude.mjs (askClaude). Liest NUR bereits gespeicherte
...
//
// KOSTET ECHTES GELD pro Aufruf (Anthropic API) - deshalb, anders als
// adaptives Lernen/Auto-Backtest, standardmäßig AUS UND läuft nur einmal
// pro Woche (montags), NIE bei jedem 5-Minuten-Cron. Braucht zusätzlich
// das ANTHROPIC_API_KEY-Secret - fehlt es, wird sauber übersprungen statt
...
function baueBriefing(alleTrades, backtestZeilen, k) {
  ...
  const tradesText = letzteTrades.map((t) => `${t.ausstiegAm}: ${t.gewinnVerlustUsdt >= 0 ? '+' : ''}${t.gewinnVerlustUsdt.toFixed(2)} USDT (${t.gewinnProzent.toFixed(2)}%) - ${t.grund}`).join('\n');
  return `Du bist ein nüchterner, ehrlicher Trading-Coach für einen KRYPTO-PAPER-TRADING-BOT (kein echtes Geld im Einsatz). Hier sind die letzten abgeschlossenen Trades:
${tradesText || 'Keine Trades.'}

Portfolio-Kennzahlen: Profit Factor ${k.profitFactor?.toFixed(2) ?? '-'}, Expectancy ${k.expectancyUsdt?.toFixed(2) ?? '-'} USDT/Trade, Recovery Factor ${k.recoveryFactor?.toFixed(2) ?? '-'}.

Letzte automatische Backtests (14 Tage, echte Kraken/Binance-Kursdaten):
${backtestZeilen.join('\n') || 'Noch keine.'}
...
export async function pruefeUndFuehreAiReview(env, cfg) {
  ...
  const letzte = await env.TRADING_STATE.get('ai-review:letzteWoche');
  ...
  if (!alleTrades.length) {
    await env.TRADING_STATE.put('ai-review:letzteWoche', aktuelleWoche);
  ...
  try {
    ...
    await env.TRADING_STATE.put('ai-review:aktuell', JSON.stringify({ text, erstelltAm: jetzt.toISOString() }));
    await notify(env, `🤖 AI Trade Review (wöchentlich):\n${text}`);
  } catch (err) {
    console.error('[trading-bot] AI Trade Review fehlgeschlagen:', err);
  }
  await env.TRADING_STATE.put('ai-review:letzteWoche', aktuelleWoche);
}
