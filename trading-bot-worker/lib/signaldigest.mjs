// Wöchentlicher Signal-Digest: fasst die Ergebnisse von Auto-Backtest,
// Strategie-Turnier, Monte-Carlo-Simulation und Korrelationsmatrix - die
// im selben Montags-Lauf schon berechnet wurden (siehe autobacktest.mjs,
// montecarlo.mjs) - zu EINER WhatsApp/Telegram-Nachricht zusammen, statt
// verstreuter Einzel-Infos, die man sich erst im Dashboard zusammensuchen
// muss. Läuft NACH pruefeUndFuehreAutoBacktest und
...
// Kauf-/Verkaufsempfehlung. Sagt nie "kaufe X", nur "diese Woche wurde X
// gemessen" - was der Nutzer daraus macht, bleibt seine eigene Entscheidung.
...
export async function pruefeUndSendeSignalDigest(env, cfg) {
  if (!cfg.autoBacktest && !cfg.monteCarlo) return; // ohne beide Quellen kein Digest möglich
  ...
  for (const symbol of cfg.symbols) {
    try {
      const mcRaw = await env.TRADING_STATE.get(`montecarlo:${symbol}`);
      if (mcRaw) {
    ...
    try {
      const tRaw = await env.TRADING_STATE.get(`turnier:${symbol}`);
      if (tRaw) {
  ...
  try {
    const kRaw = await env.TRADING_STATE.get('korrelation:matrix');
    if (kRaw) {
