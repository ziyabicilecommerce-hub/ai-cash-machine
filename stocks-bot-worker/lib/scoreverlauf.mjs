// Wöchentlicher Schnappschuss des Go-Live-Readiness-Scores (Pendant zum
// Krypto-Bot) - macht aus einer reinen Momentaufnahme einen echten Trend.
// Läuft als letzter Schritt der Montags-Wartung - liest nur bereits
// geschriebene KV-Werte plus den ohnehin vorhandenen State, kein
// zusätzlicher externer API-Call. Speichert die letzten 12 Wochen.
...
export async function pruefeUndSpeichereScoreVerlauf(env, cfg) {
  ...
  const letzte = await env.STOCKS_STATE.get('scoreverlauf:letzteWoche');
  ...
  for (const symbol of cfg.symbols) {
    try {
      ...
      console.error(`[stocks-bot] Score-Verlauf ${symbol} fehlgeschlagen:`, err);
    }
  }
  ...
  await env.STOCKS_STATE.put('scoreverlauf', JSON.stringify(verlauf));
  await env.STOCKS_STATE.put('scoreverlauf:letzteWoche', aktuelleWoche);
}
